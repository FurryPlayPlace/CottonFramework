...
public class PluginsCommand implements com.mojang.brigadier.Command<ServerCommandSource>, Predicate<ServerCommandSource>, SuggestionProvider<ServerCommandSource> {
    ...
    public int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        List<String> pluginNames = CottonAPI.get().pluginManager().getPluginsWithContainer().stream()
        ...
        context.getSource().sendFeedback(() -> Text.literal("Loaded plugins (" + pluginNames.size() +"): " + String.join(", ", pluginNames)), false);
        return 0;
    }
}